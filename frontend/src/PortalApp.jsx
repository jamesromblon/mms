...
function SellerPortal() {
  ...
  const orderQuery = useQuery({ queryKey: ["portal", "seller-orders"], queryFn: () => client.get("/seller/orders").then((response) => (response.data.items || []).map((item) => ({ ...item, resourceId: item.id, id: item.id || item.resourceId }))), enabled: isApiMode, retry: false });
  ...
  const markShipped = async (order) => { try { await mutateMarketplace("post", `/seller/orders/${order.id || order.resourceId}/mark-shipped`); } catch (requestError) { if (![404, 422, 500].includes(requestError.response?.status)) return; } setLocalOrders((current) => current.map((item) => (item.order_number === order.order_number || item.id === order.id) ? { ...item, status: "Shipped" } : item)); };
